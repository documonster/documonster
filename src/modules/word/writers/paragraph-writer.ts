...
import { renderRunProperties, renderShading, renderRun, renderBorderElement } from "./run-writer";
...
export function renderParagraphProperties(
  ...
  // Paragraph mark insertion/deletion (track changes)
  // These go inside a w:rPr wrapper within w:pPr
  if (pPr.paragraphInsertion || pPr.paragraphDeletion) {
    xml.openNode("w:rPr");
  ...
  if (pPr.markRunProperties) {
    renderRunProperties(xml, pPr.markRunProperties);
  }
...
/** Render a hyperlink. */
function renderHyperlink(xml: XmlSink, link: Hyperlink): void {
  const attrs: Record<string, string> = {};
  if (link.rId) {
    attrs["r:id"] = link.rId;
  }
...
function renderInsertedRun(xml: XmlSink, ins: InsertedRun): void {
  const attrs: Record<string, string> = {
...
function renderMovedRun(xml: XmlSink, moved: MovedFromRun | MovedToRun, wrapperTag: string): void {
  const attrs: Record<string, string> = {
  ...
  renderRun(xml, moved.run);
  xml.closeNode();
}

/** Render a paragraph child element. */
function renderParagraphChild(xml: XmlSink, child: ParagraphChild): void {
  if ("type" in child) {
    switch (child.type) {
      case "hyperlink":
        renderHyperlink(xml, child);
        return;
      case "bookmarkStart":
      ...
      case "commentReference":
        ...
        return;
      case "insertedRun":
        renderInsertedRun(xml, child);
        return;
      case "deletedRun":
        renderDeletedRun(xml, child);
        return;
      case "movedFromRun":
        renderMovedRun(xml, child, "w:moveFrom");
        return;
      case "movedToRun":
        renderMovedRun(xml, child, "w:moveTo");
        return;
  ...
  if ("content" in child && !("type" in child)) {
    renderRun(xml, child as Run);
  }
}

/** Render a w:p element. */
export function renderParagraph(xml: XmlSink, para: Paragraph): void {
  ...
  for (const child of para.children) {
    renderParagraphChild(xml, child);
  }
