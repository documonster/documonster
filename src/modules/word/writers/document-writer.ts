...
function renderSdt(xml: XmlSink, sdt: StructuredDocumentTag): void {
  xml.openNode("w:sdt");
  ...
  for (const child of sdt.content) {
    if ("type" in child) {
      if (child.type === "paragraph") {
        renderParagraph(xml, child);
      } else if (child.type === "table") {
        renderTable(xml, child);
      }
    }
  }
  xml.closeNode();
...
function renderBodyContent(xml: XmlSink, content: BodyContent): void {
  switch (content.type) {
    case "paragraph":
      renderParagraph(xml, content);
      break;
    case "table":
      renderTable(xml, content);
      break;
    case "floatingImage":
      renderFloatingImage(xml, content);
      break;
    ...
    case "checkBox":
      ...
      break;
    case "drawingShape":
      renderDrawingShape(xml, content as DrawingShape);
      break;
    case "opaqueDrawing":
...
export function renderDocument(xml: XmlSink, doc: DocxDocument): void {
...
function renderDrawingShape(xml: XmlSink, shape: DrawingShape): void {
  ...
  const docPrAttrs: Record<string, string> = { id: "1", name: shape.name ?? "Shape" };
  if (shape.altText) {
 ...
 */
export const CHART_RID_REGISTRY = new WeakMap<object, string>();

/** Auto-incrementing SDT ID counter. */
let _autoSdtIdCounter = 100000000;
function nextAutoSdtId(): number {
  return _autoSdtIdCounter++;
}

function renderChartDrawing(xml: XmlSink, content: ChartContent): void {
  const rId = CHART_RID_REGISTRY.get(content) ?? "rId0";
