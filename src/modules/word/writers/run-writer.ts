...
function renderInlineImage(xml: XmlSink, img: InlineImageContent): void {
...
/** Render a field code (PAGE, NUMPAGES, etc.). */
function renderField(xml: XmlSink, field: FieldContent): void {
  // Begin
  xml.openNode("w:r");
  if (field.formField) {
...
function renderRunContent(xml: XmlSink, content: RunContent): boolean {
  switch (content.type) {
    case "text":
...
export function renderRun(xml: XmlSink, run: Run): void {
  ...
  for (const c of run.content) {
    if (c.type === "field") {
      ...
      fieldContent.push({
        ...
        cachedValue: new Date().toISOString().split("T")[0]
      });
    } else {
  ...
  if (normalContent.length > 0) {
    ...
    for (const content of normalContent) {
      renderRunContent(xml, content);
    }
    xml.closeNode();
  }

  // Render field content (each field creates multiple runs)
  for (const fc of fieldContent) {
    if (fc.type === "field") {
      renderField(xml, fc as FieldContent);
    }
  }
}
