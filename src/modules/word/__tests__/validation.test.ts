...
import { validateDocument, Document, textParagraph } from "../index";
...
describe("Document validation", () => {
  it("valid document passes", () => {
    ...
    const result = validateDocument(Document.build(doc));
  ...
  it("maxErrors option limits error count", () => {
    ...
    const result = validateDocument(doc, { maxErrors: 1 });
    ...
    const errorsUnlimited = validateDocument(doc).issues.filter(i => i.severity === "error");
