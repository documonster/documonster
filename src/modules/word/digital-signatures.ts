 ...
 *
 * Note: Creating new signatures requires X.509 certificate handling and
 * is outside the scope of this module. Consider using a dedicated signing
 * tool for the actual signing step.
 *
 * References:
...
export function extractSignatures(
  ...
  for (const part of opaqueParts) {
    if (part.path.startsWith("_xmlsignatures/sig") && part.path.endsWith(".xml")) {
      const xmlStr = decoder.decode(part.data);
