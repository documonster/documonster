...
interface TwoCellModel {
  ...
  /**
   * `<xdr:grpSp>` grouped shapes, captured verbatim (see
   * {@link GenericEchoXform}). Without an explicit map entry for
   * "xdr:grpSp", the anchor's dispatch loop fell through to its generic
   * per-tag lookup for every descendant of the (unrecognized) group
   * wrapper too - so an `<xdr:pic>` nested inside the group was matched
   * directly and promoted to `model.picture`, silently replacing the
   * whole group with one of its own children at the wrong geometry.
   */
  group?: EchoNode;
  ...
  parseClose(name: string): boolean {
    ...
    switch (name) {
      case this.tag:
        ...
        // Only pull a child model in if THIS anchor actually parsed that
        // tag, and reset the child xform right after - these instances are
        // reused across every anchor in the drawing, so leaving a stale
        // `.model` around made the next anchor of a different kind (e.g. a
        // grpSp-only anchor following a pic anchor) silently inherit the
        // previous anchor's picture instead of its own content.
