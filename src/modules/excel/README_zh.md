...
Documonster 提供结构化图表 API、模板 raw XML 保留，以及确定性的预览渲染器。目标不是只保留图表 XML，而是能直接创建、修改、导出图表预览。
...
chart.setStyle(42);
chart.setBuiltInStyle(42); // 内置 style 索引的别名
...
预览渲染是确定性、零依赖的。浏览器 PNG 使用 canvas，Node.js PNG 使用内置基础 rasterizer。它会绘制核心图表几何、坐标轴、次坐标轴、坐标轴标题、图例、标签、marker、趋势线和误差线，适合缩略图、测试、服务端预览；不是 Excel 级 pixel-perfect 渲染器，也不是 Excel 完全一致的布局引擎。ChartEx `regionMap` 预览对已知国家使用内置小型 centroid 表和投影计算，对未知标签使用确定性 tile fallback；这是地理近似预览，不是 GIS/行政边界地图渲染器。
...
# 专有 Office CLI 验证 hook。命令参数通过
# DOCUMONSTER_OFFICE_OPEN_ARGS 提供 {input} 和 {outDir} 占位符。
...
Excel、WPS 可以用相同模式接入 CI：把生成工作簿导出成 PDF/图片，再和批准件对比。Documonster 自身保持零依赖，不内置专有渲染器。内置审计是结构 gate，不能替代真实 Office 的视觉/open-repair 验证。
...
统一入口仍是 `chartToPdf(chart, options)`（从 `documonster/pdf`）—— 自动选择路径，可用 `{ forceRaster: true }` 强制栅格（例如需要与 SVG 预览像素一致而非可选中文本时）；或直接检查 `canRenderChartExAsVectorPdf(model)` 了解路由决策。

**内置 chart style：** `chart.setStyle(1..48)`（别名 `chart.setBuiltInStyle(1..48)`）在经典 chart 上写 `<c:style val="N"/>`。这是对应 2007/2010 style 目录的轻量开关。需要 Office 2013+ 现代 style（带 `styleN.xml` / `colorsN.xml` sidecar）时，用 `worksheet.addChart({ …, chartStyle: ChartStyleModel })`。
...
- **仓库内不含 Office 真实生成的 fixture。** `src/modules/excel/__tests__/data/` 的每个 .xlsx 要么是 Documonster 自身生成要么是为回归测试手工写的最小样本。需要宿主应用兼容性覆盖时使用 opt-in 的 `DOCUMONSTER_ENTERPRISE_CORPUS_DIR` — 指向一个用户自备的 fixture 目录，`chart-oracle.integration.test.ts` 会逐个审计。参考 `docs/enterprise-corpus-manifest.example.json` 的 manifest 格式。
- **CI 没有自动化的 Excel / WPS 运行时。** CI 的 open-validation 仅 gate 到 LibreOffice 上；Excel / WPS 二进制不出现在任何 CI runner 中，GUI 驱动的这两款 app 的 validation 超出范围。`DOCUMONSTER_OFFICE_OPEN_VALIDATION` + `DOCUMONSTER_OFFICE_OPEN_ARGS` 钩子允许自建的安装了 Office 的 runner 按同一 pattern 参与这一检查。
...
```typescript
import { Workbook, excelToPdf } from "documonster";
...
```typescript
import { WorkbookReader } from "documonster";
...
```typescript
import { WorkbookWriter } from "documonster";
...
```typescript
import { WorkbookWriter, WorkbookReader } from "documonster";
