...
统一入口仍是 `chartToPdf(chart, options)`（从 `@cjnoname/excelts/pdf`）—— 自动选择路径，可用 `{ forceRaster: true }` 强制栅格（例如需要与 SVG 预览像素一致而非可选中文本时）；或直接检查 `canRenderChartExAsVectorPdf(model)` 了解路由决策。
