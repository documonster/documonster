/**
 * Canonical XML character encoding / decoding primitives (Layer 0).
 *
 * These functions are dependency-free so every module — including `pdf`,
 * which may not depend on the `xml` module per the layer rules — can share a
 * single, correct implementation instead of carrying a hand-rolled `.replace`
 * chain that silently drifts on the edge cases (XML 1.0-illegal control
 * characters, lone UTF-16 surrogate halves, the `0xFFFE` / `0xFFFF`
 * noncharacters).
 *
 * `@xml/encode` re-exports everything here and adds the name/comment
 * validators that need `XmlError`. Prefer importing from `@xml/encode` inside
 * modules that already depend on `xml`; import from here only when a module
 * cannot reach the `xml` layer (currently just `pdf`).
 */
 ...
 */
export function isLegalXmlChar(code: number): boolean {
 ...
 *
 * Shared so `chart-utils.ts` and any other module that needs the
 * same sanitisation can use a single implementation instead of
 ...
 * them first (same sanitisation as {@link xmlEncode}) so CDATA output
 * stays well-formed.
 *
...
export function encodeCData(text: string): string {
  return "<![CDATA[" + stripXmlIllegalChars(text).split("]]>").join("]]]]><![CDATA[>") + "]]>";
}
