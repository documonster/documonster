 ...
 *
 * Setting `EXCELTS_ENTERPRISE_CORPUS_DIR=/path` additionally appends a
 ...
 *
 *   EXCELTS_ENTERPRISE_CORPUS_DIR=/path/to/corpus \
 *     EXCELTS_LIBREOFFICE_OPEN_VALIDATION=1 \
...
/**
 * How ExcelTS stacks up against the leading alternatives. The matrix
...
async function renderCorpusSection(root: string): Promise<string> {
  ...
  lines.push(
    "_Full audit / LibreOffice open-validation runs live in" +
      ...
      " Set `EXCELTS_ENTERPRISE_CORPUS_DIR` + `EXCELTS_LIBREOFFICE_OPEN_VALIDATION=1`" +
...
async function main(): Promise<void> {
  const outPath = process.env.EXCELTS_COMPATIBILITY_REPORT_OUT
    ? resolve(process.env.EXCELTS_COMPATIBILITY_REPORT_OUT)
    : DEFAULT_OUT;
  const lines: string[] = [];
  lines.push("# ExcelTS chart compatibility report");
  lines.push("");
  ...
  lines.push(
    "This document mirrors the per-type capability grid in" +
      ...
      " `EXCELTS_ENTERPRISE_CORPUS_DIR` environment variable is set." +
  ...
  lines.push("");
  lines.push(
    "ExcelTS ships a **zero-dependency deterministic preview renderer**" +
  ...
  const corpusRoot = process.env.EXCELTS_ENTERPRISE_CORPUS_DIR;
  if (corpusRoot && existsSync(corpusRoot)) {
    ...
    lines.push("");
    lines.push(
      "_No corpus directory supplied._ Set `EXCELTS_ENTERPRISE_CORPUS_DIR`" +
